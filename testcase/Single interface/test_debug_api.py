...
class TestUserManager:
    ...
    @allure.story(next(c_id) + "查询用户-边界与反向测试")
    @pytest.mark.run(order=5)
    @pytest.mark.parametrize('base_info,testcase', get_testcase_yaml("./testcase/Single interface/boundary_query.yaml"))
    def test_boundary_query(self, base_info, testcase):
        allure.dynamic.title(testcase['case_name'])
        RequestBase().specification_yaml(base_info, testcase)

    @allure.story(next(c_id) + "新增用户-边界与反向测试")
    @pytest.mark.run(order=6)
    @pytest.mark.parametrize('base_info,testcase', get_testcase_yaml("./testcase/Single interface/boundary_add.yaml"))
    def test_boundary_add(self, base_info, testcase):
        allure.dynamic.title(testcase['case_name'])
        RequestBase().specification_yaml(base_info, testcase)

    @allure.story(next(c_id) + "删除用户-边界与反向测试")
    @pytest.mark.run(order=7)
    @pytest.mark.parametrize('base_info,testcase', get_testcase_yaml("./testcase/Single interface/boundary_delete.yaml"))
    def test_boundary_delete(self, base_info, testcase):
        allure.dynamic.title(testcase['case_name'])
        RequestBase().specification_yaml(base_info, testcase)
