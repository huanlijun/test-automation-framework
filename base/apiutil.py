...
class RequestBase:
    ...
    def specification_yaml(self, base_info, test_case):
        ...
        try:
            ...
            # 处理响应时间断言阈值（单位：秒）
            response_time_limit = test_case.pop('response_time', None)
            # 处理断言
            ...
            try:
                ...
                if response_time_limit is not None:
                    ...
                    allure.attach(
                        f'响应时间：{res_time:.3f}s，阈值：{response_time_limit}s',
                        '响应时间断言',
                        allure.attachment_type.TEXT
                    )
                    self.asserts.assert_response_time(res_time, float(response_time_limit))
